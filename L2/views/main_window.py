...
class PaginationPanel(QWidget):
    ...
    def set_info(self, page: int, total_pages: int, on_page: int, total: int) -> None:
        ...
        self.records_label.setText(f"Записей на странице: {on_page} | Всего записей: {total}")
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.tree_model.setHorizontalHeaderLabels(["Книга", "Значение"])
    ...
    def set_tree_rows(self, rows: list[dict[str, str]]) -> None:
        ...
        for row in rows:
            root = QStandardItem(row["title"])
            root.appendRow([QStandardItem("ФИО автора"), QStandardItem(row["author"])])
            root.appendRow([QStandardItem("Издательство"), QStandardItem(row["publisher"])])
            root.appendRow([QStandardItem("Число томов"), QStandardItem(row["volumes"])])
            root.appendRow([QStandardItem("Тираж"), QStandardItem(row["circulation"])])
            root.appendRow([QStandardItem("Итого томов"), QStandardItem(row["total_tomes"])])
