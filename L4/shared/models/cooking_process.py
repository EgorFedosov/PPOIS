"""Finite-state cooking process controller."""
...
class CookingProcess:
    """Scenario for cooking fried eggs."""
    ...
    def break_eggs(self) -> str:
        ...
        if not self.pan.is_hot:
            raise InvalidStateTransitionError("Сковорода холодная. Сначала разогрейте ее.")
    ...
    def add_spices(self) -> str:
        ...
        if self.state not in (CookingState.COOKING, CookingState.SEASONING):
            raise InvalidStateTransitionError(
                "Приправы можно добавить только после разбивания яиц."
            )
    ...
    def mix_and_serve(self) -> str:
        ...
        if self.egg.state != EggState.FRIED:
            raise InvalidStateTransitionError("Нельзя подать сырое блюдо.")
