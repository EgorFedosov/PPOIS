...
class Menu:
    ...
    def run(self) -> None:
        while True:
            print("\n=== Модель приготовления яичницы ===")
            print(f"Состояние: {self.process.state.value}")
            print(f"Плита: {'включена' if self.process.stove.is_on else 'выключена'}")
